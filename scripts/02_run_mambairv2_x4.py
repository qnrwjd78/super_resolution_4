...
def main() -> None:
    ...
    default_opt = str((script_dir / "options" / "mambairv2_x4.yml").resolve())

    ap = argparse.ArgumentParser(
        description="Run MambaIRv2 x4 on VAL_JSON LR-only (symlink stage), optionally write result.json."
    )
    ap.add_argument("--json", default=default_val_json, help="Path to val_fixed.json (only 'lr' is used)")
    ap.add_argument("--repo_dir", default=default_repo_dir, help="Path to MambaIR repo")
    ...
    ap.add_argument("--opt", default=os.environ.get("OPT", default_opt), help="Path to MambaIRv2 option YAML (template)")
    ap.add_argument("--name", default=os.environ.get("NAME", "MambaIRv2_x4"))
    ap.add_argument("--weight", default=os.environ.get("WEIGHT", ""), help="Path to weight (.pth) (empty = repo default)")
    ...
    weight = (
        ...
        if weight_raw
        else (repo_dir / "experiments" / "pretrained_models" / "mambairv2_classicSR_Base_x4.pth")
    ).resolve()
