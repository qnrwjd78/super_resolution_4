...
def main() -> None:
    ...
    default_repo_dir = str((sr_dir / "repos" / "HAT").resolve())
    default_opt = str((script_dir / "options" / "hat_x4.yml").resolve())

    ap = argparse.ArgumentParser(description="Run HAT x4 on VAL_JSON LR-only (symlink stage), optionally write result.json.")
    ap.add_argument("--json", default=default_val_json, help="Path to val_fixed.json (only 'lr' is used)")
    ap.add_argument("--repo_dir", default=default_repo_dir, help="Path to HAT repo")
    ...
    ap.add_argument("--opt", default=os.environ.get("OPT", default_opt), help="Path to HAT option YAML (template)")
    ap.add_argument("--name", default=os.environ.get("NAME", "HAT_SRx4_ImageNet-LR"))
    ap.add_argument("--weight", default=os.environ.get("WEIGHT", ""), help="Path to HAT weight (.pth) (empty = repo default)")
    ap.add_argument("--tile_size", type=int, default=None, help="Override tile_size in opt (optional)")
    ap.add_argument("--tile_pad", type=int, default=None, help="Override tile_pad in opt (optional)")
    ...
    weight = (
        ...
        if weight_raw
        else (repo_dir / "experiments" / "pretrained_models" / "HAT_SRx4_ImageNet-pretrain.pth")
    ).resolve()
    ...
    if args.tile_size is not None:
        force_yml.append(f"tile:tile_size={int(args.tile_size)}")
    if args.tile_pad is not None:
        force_yml.append(f"tile:tile_pad={int(args.tile_pad)}")

    cmd = [
        sys.executable, "hat/test.py",
        "-opt", str(opt_path),
        "--force_yml",
        *force_yml,
    ]
