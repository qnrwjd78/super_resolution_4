...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default=None, help='output folder to save results (override default)')
    ...
    if folder is None or str(folder).strip() == "":
        raise ValueError("Input folder is not set. Provide --folder_lq (and --folder_gt for paired tasks).")
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        if img_gt is None:
            out_path = os.path.join(save_dir, os.path.basename(path))
        else:
            out_path = os.path.join(save_dir, f'{imgname}_SwinIR.png')
...
def setup(args):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        ...
        if args.folder_gt is None or str(args.folder_gt).strip() == "":
...
def get_image_pair(args, path):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        folder_gt = getattr(args, "folder_gt", None)

        # LQ-only mode: if folder_gt is not provided, treat `path` as the LQ image itself.
        if folder_gt is None or str(folder_gt).strip() == "":
            img_lq = cv2.imread(path, cv2.IMREAD_COLOR)
            if img_lq is None:
                raise FileNotFoundError(f"Failed to read LQ image: {path}")
        ...
        if img_lq is None:
            raise FileNotFoundError(f"Failed to read LQ image: {img_lq_path}")
        img_lq = img_lq.astype(np.float32) / 255.0
