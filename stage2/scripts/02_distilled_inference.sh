...
# FLUX.2 [klein-9b distilled] image-to-image inference launcher.
#
# This script is intentionally fixed to:
#   - model: flux.2-klein-9b
#   - mode : i2i (input image(s) required)
#   - sampling params: num_steps=4, guidance=1.0 (distilled requirement)
#
# Examples:
#   ./02_distilled_inference.sh \
...
MODEL_ENV_VAR="KLEIN_9B_MODEL_PATH"
...
usage() {
  cat <<'EOF'
Usage:
  ./02_distilled_inference.sh [options] [-- <extra fire args>]
...
Notes:
  This distilled launcher always runs with:
    --num_steps=4
    --guidance=1.0
...
CLI_ARGS+=("--num_steps=4" "--guidance=1.0")
...
echo "PROMPT     : $PROMPT"
echo "DISTILLED  : num_steps=4 guidance=1.0"
echo
